...
pre-processing of data. calculation of MPE and FFC for PARTICIPANT #0136.
This participant had so many repeated trials that the matlab file became too big
...
final_folder = '/Users/mg/Desktop/MOVE/MOVE_Analysis_MG/KINARM/MPE_Block_new'
final_list = listdir(final_folder)

# the exclusive participant
vp = '0136'

# C3D Import
# define kinarm data path with unzipped .mat files
subject_path = data_folder + 'MOVE_0136/KINARM/'
files = listdir(subject_path)
files

# make sure to select matlab file, make sure in mat file is just one experiment
# select file index/number
vpk = vp + '*' + 'MPEclean.csv'

# execute only if file does not exist yet
if fnmatch.filter(final_list, vpk):
    print(vpk)
else:
    print('todo ' + vpk)
    ...
    # make sure to select matlab file, make sure in mat file is just one experiment
    # select file index/number
    nr_file = files.index('data_kine1.mat')

    # import data file via scipy
    c3d = sio.loadmat(subject_path + files[nr_file])

    # extracting the kinarm dataframe; look at key in c3d
    mdata = c3d['data_kine1']

    # if the dataframe has several trials/rows, you can define them here
    datensatz, nTotalTrials = mdata.shape
    print('Total Trials: ', nTotalTrials)
    print('Datensaetze: ', datensatz)
    ...
    # merging several trials in one dataframe
    appended_df = []
    ...
    # I am looking at the last trial in task 2 (if participants had no error trial)
    x_offset2 = (mdata['TARGET_TABLE'][0][539][0][0]['X_GLOBAL'][1][0]) / 100
    y_offset2 = (mdata['TARGET_TABLE'][0][539][0][0]['Y_GLOBAL'][1][0]) / 100
    ...
    for trial in np.arange(nTotalTrials):
        ...
        # clamp trials sind automatisch adaptation, (falsch)
        ...
        # constants are here in matlab file:
        # x: TARGET_TABLE/X_GLOBAL/2. value divided by 100
        # y: TARGET_TABLE/Y_GLOBAL/2. value divided by 100
...
# which is the last trial?
last_trial_part_1 = df.Trial.max()
# %% load task 2
...
if 2 > 1:
    ...
    for trial in np.arange(nTotalTrials):
        ...
        # now make a data frame, setting the time stamps as the index
        df2 = pd.DataFrame(np.concatenate([ndata[c] for c in columns], axis=1), columns=columns)

        # adding columns
        df2['Milliseconds'] = ms
        df2['Trial'] = trial + last_trial_part_1 + 1
        df2['Events'] = 0
        ...
        eventcount = 0
        for i in events:  # add number of position in list to events column
            df2.loc[df2['Milliseconds'] >= round(i), 'Events'] = eventcount
            eventcount = eventcount + 1
        ...
        df2['Event_Label'] = eventlabels

        # keep only data after middle is left
        df2 = df2.loc[(df2['Event_Label'] == 'LEAVE_MIDDLE') |
                      (df2['Event_Label'] == 'MIDDLE_LIGHT_ON_ALREADY_LEFT'), :]
        ...
        taskprotocol = experimentlist['TASK_PROTOCOL']
        taskprotocol = taskprotocol[0, 0][0]
        df2['Task_Protocol'] = taskprotocol
        part = taskprotocol[-5:]
        ...
        df2['Block_Run_Count'] = block_run_count

        if part == 'part2':
            df2['Block_Run_Count'] = block_run_count + 31
        elif part == 'er_V2':
            df2['Block_Run_Count'] = block_run_count + 60
        elif part == 'er_V1':
            df2['Block_Run_Count'] = block_run_count + 60
        ...
        if tp > 10 and tp < 20 and part == 'part1':
            ...
            df2['Phase'] = 'Adaptation'

        # force field direction, 0 = + 135°, 1 = -135°
        if tp > 10:
            df2['Cue_Target'] = tp - 10
        else:
            df2['Cue_Target'] = tp
        ...
        # for this we need to correct by a constants which are different for each participant
        # constants are here in matlab file:
        # x: TARGET_TABLE/X_GLOBAL/2. Wert divided by 100
        # y: TARGET_TABLE/Y_GLOBAL/2. Wert divided by 100

        if part == 'part1':
            df2['X'] = (df2['Right_HandX'] - x_offset) * 100
            df2['Y'] = (df2['Right_HandY'] - y_offset) * 100
        else:
            df2['X'] = (df2['Right_HandX'] - x_offset2) * 100
            df2['Y'] = (df2['Right_HandY'] - y_offset2) * 100

        # Deviation
        # Sign_Deviation = deviation is positive or negative
        if tp in [1, 2, 11, 12, 5, 6, 15, 16]:

            df_out = df2.loc[(df2.Y < -12.625) |
                             (df2.Y > 12.625), :]
            if not df_out.empty:
                idx = df_out.index[0]
                df_in = df2.loc[:idx]
            ...
            df2['Deviation'] = abs_X
            if abs_X.empty:
                df2['Sign_Deviation'] = 0
                max_abs = 0
                sign = 0
            else:
                max_abs = max(abs_X)
                idx_max_abs_X = np.where(np.abs(df2.X) == max_abs)[0][0]
                sign = np.sign(df2.X[df2.index[idx_max_abs_X]])
                df2['Sign_Deviation'] = sign

        else:
            df_out = df2.loc[(df2.X < -12.625) |
                             (df2.X > 12.625), :]
            if not df_out.empty:
                idx = df_out.index[0]
                df_in = df2.loc[:idx]
            ...
            if abs_Y.empty:
                ...
                max_abs = max(abs_Y)
                idx_max_abs_Y = np.where(np.abs(df2.Y) == max_abs)[0][0]
                sign = np.sign(df2.Y[df2.index[idx_max_abs_Y]])
                df2['Sign_Deviation'] = sign

        # force field direction
        if version == '1':
            if tp in [2, 3, 5, 7, 12, 13, 15, 17]:
                df2['Force_Direction'] = 0
            else:
                df2['Force_Direction'] = 1
            if tp in [1, 4, 5, 7, 11, 14, 15, 17]:
                df2['MPE_new'] = max_abs * sign * -1
            else:
                df2['MPE_new'] = max_abs * sign
        else:
            if tp in [2, 3, 5, 7, 12, 13, 15, 17]:
                df2['Force_Direction'] = 1
            else:
        ...
        # adding trail to dataframe
        appended_df2.append(df2)

    # merging all trials into one dataframe
    df2 = pd.concat(appended_df2, axis=0)
...
df_reach.to_csv(r'/Users/mg/Desktop/MOVE/MOVE_Analysis_MG/KINARM/XY_trial/' +
...
df_MPE_new.to_csv(r'/Users/mg/Desktop/MOVE/MOVE_Analysis_MG/KINARM/MPE_Block_new/' +
...
df_MPE_trial.to_csv(r'/Users/mg/Desktop/MOVE/MOVE_Analysis_MG/KINARM/MPE_trial/' +
                    vp + group + version + 'MPEclean_trial.csv')
...
df_clamp_trial.to_csv(r'/Users/mg/Desktop/MOVE/MOVE_Analysis_MG/KINARM/CLAMP_Trial/' +
...
df_CLAMP.to_csv(r'/Users/mg/Desktop/MOVE/MOVE_Analysis_MG/KINARM/CLAMP_Block/' +
                vp + group + version + '.csv')
